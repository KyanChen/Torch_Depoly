...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
                 ...
                 norm_layer=None,out_keys=None,in_channels=3):
...
def _resnet(arch, block, layers, pretrained, progress,num_classes=1000,in_channels=3, **kwargs):
    model = ResNet(block, layers,num_classes,in_channels=in_channels, **kwargs)
    if pretrained:
        ...
        if in_channels !=3:
            ...
            keys = [x for x in keys if 'conv1' in x]
            for key in keys:
                del state_dict[key]
        ...
        # print('resume model from resnet50:{0}'.format(state_dict.keys()))
    return model
...
def get_resnet(name='resnet50',pretrained=True,progress=True,
    ...
    if name=='resnet18':
        return _resnet18(name=name,pretrained=pretrained,progress=progress,
...
if __name__=='__main__':
    model=get_resnet('resnet50',pretrained=False,num_classes=None,in_channels=38,
                     out_keys=('block2','block3','block4','block5','block6'))
    x=torch.rand([2,38,512,512])
